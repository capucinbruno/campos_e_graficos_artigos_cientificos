# -*- coding: utf-8 -*-
# ══════════════════════════════════════════════════════════════════════════════════════════════════
# CONFIG DO SCRIPT s38 — EDITE AQUI (não no settings.local). Aplicada no início do main() via
...
GLOBO_3D_LON_INICIAL = -73.0
GLOBO_3D_LAT_INICIAL = -32.0
GLOBO_3D_LON_FINAL   = -73.0
...
# ── s38 - Globo 3D animado (mídia) estilo Ben Noll: voo da câmera + evolução temporal ──────────────
# Gera um MP4 de uma variável meteorológica sobre um globo flutuante que gira/voa enquanto o campo
# evolui no tempo. Modo (reanálise/forecast) decidido pelas datas. Motor genérico em
# app/src/uteis/globo_3d_anim.py. Saída: Saida/s38_GLOBO_MIDIA/{REANALISE|FORECAST/<MODELO>}/s38_<var>.mp4
...
SCRIPT_DESC = 's38 - Globo 3D animado (midia) estilo Ben Noll'
...
def main():
    ...
    cache_params = {
        ...
        'script_version': '2.58',  # corrente de jato disponivel em qualquer campo (master unico GLOBO_3D_JATO)
    }
