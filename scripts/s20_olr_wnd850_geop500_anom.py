"""s20 - Anomalia de OLR + Vento 850 hPa + Geopotencial 500 hPa.

Pipeline de dados:
  - OLR: PSL/NOAA CPC Blended OLR (shaded BrBG_r)
  - Vento 850 hPa: ERA5/GDAS + climatologia PSL/NOAA (quiver)
  - Geopotencial 500 hPa: ERA5/GDAS + climatologia PSL/NOAA (contorno preto)

Saida:
    - Mapas PNG em Saida/s20_OLR_WND850_GEOP500/ (um por regiao em LST_AREAS_S20)
...
# ---------------------------------------------------------------------------
SCRIPT_ID = Path(__file__).stem.split('_')[0]  # 's20'
...
def _get_area_list() -> list[str]:
    if hasattr(settings, 'LST_AREAS_S20'):
        return list(settings.LST_AREAS_S20)
...
def _load_field(nc_path: Path, var_name: str) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Carrega campo 2D de NetCDF, normaliza lon 0-360, retorna (data_cyc, lon_cyc, lat)."""
...
def main():
    ...
    output_dir = Path(settings.DIR_OUTPUT) / f'{SCRIPT_ID}_OLR_WND850_GEOP500'
    ...
    output_files = [str(output_dir / f'olr_wnd850_geop500_{area}.png') for area in lst_areas]
    ...
    logger.info('📊 Gerando {} mapas — OLR + vento 850 hPa + geopotencial 500 hPa', len(lst_areas))
    logger.info('=' * 80)

    # Etapa 1: OLR — download e processamento inline (mesmo padrão do s05)
    logger.info('Etapa 1: Preparando anomalia de OLR')
    ...
    if arquivo_cobre_periodo(olr_path, start_date, end_date):
        logger.info('Arquivo OLR local já cobre o período solicitado — pulando download')
    else:
        if olr_path.exists():
            logger.info('Arquivo OLR local não cobre {} a {} — re-baixando', settings.DATA_INICIAL, settings.DATA_FINAL)
    ...
    for area in lst_areas:
        ...
        # Título
        titulo = f'Anom. OLR + Vento 850hPa + Geopotencial 500hPa (De {dt_ini_str} a {dt_fim_str})'
        ...
        filename_fig = output_dir / f'olr_wnd850_geop500_{area}.png'
