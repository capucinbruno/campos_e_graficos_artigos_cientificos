"""s19 - Anomalia de Temperatura 850 hPa + Vento 850 hPa + Geopotencial 500 hPa.

Pipeline de dados:
  - Temperatura 850 hPa: ERA5/GDAS + climatologia PSL/NOAA (shaded)
  - Vento 850 hPa: ERA5/GDAS + climatologia PSL/NOAA (quiver)
  - Geopotencial 500 hPa: ERA5/GDAS + climatologia PSL/NOAA (contorno preto)

Saida:
    - Mapas PNG em Saida/s19_TMP850_WND850_GEOP500/ (um por regiao em LST_AREAS_S19)
...
# ---------------------------------------------------------------------------
SCRIPT_ID = Path(__file__).stem.split('_')[0]  # 's19'
...
GEOP500_FILE_NAME = 'geop500.nc'

# Temperatura 850 hPa — níveis e ticks
TMP_LEVELS = np.arange(-5, 5.5, 0.5)
TMP_TICKS = np.arange(-5, 6, 1)

# Geopotencial 500 hPa — contorno preto (exclui 0 para não poluir o mapa)
GEOP_CONTOUR_LEVELS = np.concatenate([np.arange(-100, 0, 10), np.arange(10, 110, 10)])

# Parâmetros de quiver por área (mesmo do s12/s18)
...
def _get_area_list() -> list[str]:
    if hasattr(settings, 'LST_AREAS_S19'):
        return list(settings.LST_AREAS_S19)
...
def _load_field(nc_path: Path, var_name: str) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Carrega um campo 2D de um NetCDF, normaliza lon 0-360, retorna (data_cyc, lon_cyc, lat)."""
...
def main():
    ...
    output_dir = Path(settings.DIR_OUTPUT) / f'{SCRIPT_ID}_TMP850_WND850_GEOP500'
    ...
    output_files = [str(output_dir / f'tmp850_wnd850_geop500_{area}.png') for area in lst_areas]
    ...
    logger.info('📊 Gerando {} mapas — temperatura + vento 850 hPa + geopotencial 500 hPa', len(lst_areas))
    logger.info('=' * 80)

    # Etapa 1: temperatura 850 hPa → tmp850.nc
    logger.info('Etapa 1: Preparando anomalia de temperatura 850 hPa')
    plot_tmp850_anom()

    # Etapa 2: vento 850 hPa → wind850_anom.nc
    logger.info('Etapa 2: Preparando anomalia de vento 850 hPa')
    plot_wind850_anom()

    # Etapa 3: geopotencial 500 hPa → geop500.nc
    logger.info('Etapa 3: Preparando anomalia de geopotencial 500 hPa')
    plot_geop500_anom()
    ...
    tmp_cyc, lon_tmp_cyc, lat_tmp = _load_field(tmp_file, 'tmp')

    # Carregar geopotencial 500 hPa
    geop_file = dados_dir / GEOP500_FILE_NAME
    if not geop_file.exists():
        raise FileNotFoundError(f'Arquivo esperado não encontrado: {geop_file}')
    geop_cyc, lon_geop_cyc, lat_geop = _load_field(geop_file, 'hgt')
    ...
    for area in lst_areas:
        ...
        im = ax.contourf(
            ...
            levels=TMP_LEVELS, cmap=cmap, extend='both',
            transform=data_transform, zorder=2,
        )
        # Contorno preto: anomalia de geopotencial 500 hPa
        ax.contour(
            lon_geop_cyc, lat_geop, geop_cyc,
            levels=GEOP_CONTOUR_LEVELS,
            colors='black', linewidths=1.4,
            transform=data_transform, zorder=10,
        )
        ...
        # Colorbar (apenas temperatura — geopotencial não tem colorbar)
        if is_polar and area != 'globo_3d':
            cbar = plt.colorbar(im, ax=ax, pad=0.05, fraction=0.04, ticks=TMP_TICKS)
            ...
            cbar = plt.colorbar(
                ...
                location='bottom', extend='both', orientation='horizontal', ticks=TMP_TICKS,
            )
            cbar.set_label(label='°C', size=18)
            cbar.ax.tick_params(labelsize=20)
        else:
            cax = make_axes_locatable(ax).append_axes('right', size='3%', pad=0.05, axes_class=plt.Axes)
            cbar = plt.colorbar(im, cax=cax, pad=0.02, fraction=0.02375, extend='both', ticks=TMP_TICKS)
            cbar.set_label(label='°C', size=18)
            cbar.ax.tick_params(labelsize=20)

        # Título
        titulo = f'Anom. Temperatura 850hPa + Vento 850hPa + Geopotencial 500hPa (De {dt_ini_str} a {dt_fim_str})'
        ...
        filename_fig = output_dir / f'tmp850_wnd850_geop500_{area}.png'
