"""s25 - Anomalia OLR (shaded) + Geopotencial 250 hPa (contornos) + WAF + Vento Divergente.

Pipeline:
  1. Anomalia OLR em shaded (BrBG_r) — PSL/NOAA CPC
  2. Contornos pretos de anomalia de geopotencial 250 hPa
  3. Vetores WAF (Rossby Wave Activity Flux, Takaya & Nakamura 2001)
  4. Vento divergente 200 hPa (chi200 < 0, faixa -20° a 20°, branco com contorno preto)

Dados de entrada:
    - ERA5/GDAS: geopotencial 250 hPa (via plot_rossby_waf)
    - PSL/NOAA: olr.day.anom.nc (CPC Blended OLR 2.5 graus)
    - ERA5/GDAS + PSL: chi200 (via plot_chi200) — mascara e vetores do vento divergente
    - Climatologias: hgt250, uwnd250, vwnd250 (arquivos fixos em Entrada/)

Saida:
    - Mapas PNG em Saida/s25_OLR_ANOM_DIV_ROSSBY_WAF/
...
# ---------------------------------------------------------------------------
SCRIPT_ID = Path(__file__).stem.split('_')[0]  # 's25'
...
def _get_area_list() -> list[str]:
    if hasattr(settings, 'LST_AREAS_S25'):
        return list(settings.LST_AREAS_S25)
...
def _ask_lag(olr_ini: str, olr_fim: str) -> tuple[bool, str, str]:
    """Pergunta ao usuario se deseja defazar Geop250+WAF em relacao ao OLR+vento divergente.

    Retorna (usar_lag, lag_ini_str, lag_fim_str).
    Se o usuario nao quiser lag, lag_ini_str e lag_fim_str sao iguais ao periodo OLR.
    """
    print()
    print(f'  Periodo OLR + Vento Divergente: {olr_ini} a {olr_fim}')
    resp = input(
        '  Deseja defazar OLR + Vento Divergente com Geopotencial 250 hPa + WAF? [s/n]: '
    ).strip().lower()

    if resp not in ('s', 'sim', 'y', 'yes'):
        return False, olr_ini, olr_fim
...
def main():
    ...
    output_dir = Path(settings.DIR_OUTPUT) / f'{SCRIPT_ID}_OLR_ANOM_DIV_ROSSBY_WAF'
    input_dir = Path(settings.DIR_INPUT)
    dados_dir = Path(settings.DIR_DADOS)

    olr_ini_str = str(settings.DATA_INICIAL)
    olr_fim_str = str(settings.DATA_FINAL)

    # ---- Pergunta de defasagem (antes do cache para que o lag entre nos params) ----
    usar_lag, lag_ini_str, lag_fim_str = _ask_lag(olr_ini_str, olr_fim_str)

    cache_params = {
        'DATA_INICIAL': olr_ini_str,
        'DATA_FINAL': olr_fim_str,
        'areas': lst_areas,
        'script_version': '1.1',
    ...
    output_files = [str(output_dir / f'olr_anom_div_rossby_waf_{area}.png') for area in lst_areas]

    if check_cache_valid(SCRIPT_ID, cache_params, output_files):
        logger.info('CACHE VALIDO! Execucao ja foi realizada com os mesmos parametros.')
        logger.info(f'   OLR + Vento Div: {olr_ini_str} a {olr_fim_str}')
        if usar_lag:
    ...
    if usar_lag:
        logger.info(f'Geop250 + WAF:   {lag_ini_str} a {lag_fim_str}')
    logger.info(f'Gerando {len(lst_areas)} mapas — OLR + WAF + vento divergente')

    start_date_olr = np.datetime64(olr_ini_str)
    end_date_olr = np.datetime64(olr_fim_str)
    ...
    if usar_lag:
        # Sobrescreve temporariamente as datas do settings para plot_rossby_waf()
        settings.DATA_INICIAL = lag_ini_str
        settings.DATA_FINAL = lag_fim_str
    plot_rossby_waf()
    if usar_lag:
        settings.DATA_INICIAL = olr_ini_str
        settings.DATA_FINAL = olr_fim_str
    gc.collect()

    # ---- Etapa 2: OLR (PSL/NOAA) — sempre usa periodo OLR ----
    logger.info(f'Etapa 2: Preparando anomalia de OLR ({olr_ini_str} a {olr_fim_str})...')
    olr_path = dados_dir / OLR_FILE_NAME
    if arquivo_cobre_periodo(olr_path, start_date_olr, end_date_olr):
        logger.info('Arquivo OLR local ja cobre o periodo — pulando download')
    else:
        if olr_path.exists():
            logger.info('Arquivo OLR nao cobre o periodo — re-baixando')
    ...
    validar_cobertura_temporal(ds_olr, start_date_olr, end_date_olr, nome='OLR')
    subset_olr = ds_olr.sel(time=slice(start_date_olr, end_date_olr))
    ...
    olr_cyc, lon_olr_cyc = add_cyclic_point(da_olr, coord=da_olr['lon'])
    gc.collect()

    # ---- Etapa 3: CHI200 (mascara + vetores do vento divergente) — periodo OLR ----
    logger.info(f'Etapa 3: Calculando CHI200 ({olr_ini_str} a {olr_fim_str})...')
    plot_chi200()
    gc.collect()
    ...
    # ---- Etapa 4: Carregar WAF (periodo do lag se ativo) ----
    ...
    olr_ini_fmt = datetime.strptime(olr_ini_str, '%Y-%m-%d').strftime('%d-%m-%y')
    olr_fim_fmt = datetime.strptime(olr_fim_str, '%Y-%m-%d').strftime('%d-%m-%y')
    ...
    for area in lst_areas:
        ...
        # --- Camada 1: OLR shaded (BrBG_r, sem contorno) ---
        im = ax.contourf(
        ...
        if is_polar and area != 'globo_3d':
            cbar = plt.colorbar(im, ax=ax, pad=0.05, fraction=0.04, ticks=OLR_TICKS)
            ...
            cbar = plt.colorbar(im, cax=cax, pad=0.02, fraction=0.02375, extend='both', ticks=OLR_TICKS)
            ...
            cbar = plt.colorbar(
                ...
                location='bottom', extend='both', orientation='horizontal', ticks=OLR_TICKS,
            )
            cbar.set_label(label='W/m$^2$', size=18)
            cbar.ax.tick_params(labelsize=20)
        ...
        if usar_lag:
            titulo = (
                f'OLR + Vento Div: {olr_ini_fmt} a {olr_fim_fmt} | '
                f'Geop250 + WAF: {lag_ini_fmt} a {lag_fim_fmt}\n'
                f'Anomalia OLR (shaded) + Geopotencial 250 hPa (linhas) + WAF + Vento Divergente chi200<0'
            )
        else:
            titulo = (
                f'Anomalia OLR (shaded) + Geopotencial 250 hPa (linhas) +\n'
                f'WAF + Vento Divergente chi200<0 (De {olr_ini_fmt} a {olr_fim_fmt})'
            )
        ...
        filename_fig = output_dir / f'olr_anom_div_rossby_waf_{area}.png'
