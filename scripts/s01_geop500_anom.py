"""s01 - Anomalia de Geopotencial 500 hPa (ERA5/GDAS + Climatologia PSL).

Pipeline de dados:
  - Periodo recente (ultimos 7 dias): GDAS via NOMADS Grib Filter
  - Periodo mais antigo: ERA5 via Copernicus CDS
  - Climatologia: PSL/NOAA via Playwright (cache local por periodo MM-DD)

Saida:
    - Mapas PNG em Saida/s01_GEOP500/ (um por regiao configurada em LST_AREAS_S01)
...
from app.src.uteis.plot_geop500 import main as plot_geop500

# ---------------------------------------------------------------------------
# Workaround: cartopy 0.25 + matplotlib 3.10 — GeometryCollection não subscritável.
# _rings_to_multi_polygon retorna GeometryCollection em geometrias fragmentadas;
# patching transform_path_non_affine para retornar path vazio como fallback.
# ---------------------------------------------------------------------------
import numpy as _np
...
def main():
    ...
    output_dir = Path(settings.DIR_OUTPUT) / f'{SCRIPT_ID}_GEOP500'
    ...
    output_files = [str(output_dir / f'geop500_{area}.png') for area in lst_areas]
    ...
    logger.info('📊 Gerando {} mapas de anomalia GEOP500', len(lst_areas))
    logger.info('=' * 80)
    ...
    ds1 = load_dataset(str(dados_dir / 'geop500.nc'))
    ...
    lat = da['lat'].values
    hgt, lon = add_cyclic_point(da.values, coord=da['lon'].values)
    ...
    for area in lst_areas:
        ...
        # hgt e lon já preparados fora do loop (0-360, costura em 0°)
        ...
        if area == 'enso':
            ...
            for x, y, label, color, stroke_color in _enso_labels:
        ...
        if area == 'mjo':
            for i, (x, num) in enumerate(zip([-135.25, -115.25, -95.25, -75.25, -55.25, -35.25, -15.25, 4.75], range(1, 9))):
        ...
        # Fundo branco substitui cfeature.OCEAN — evita costura do polígono Natural Earth em 180°
        ...
        titulo = f'Anomalia de Alt. Geopotencial 500mb (De {dt_ini} a {dt_fim})'
        ...
        filename_fig = output_dir / f'geop500_{area}.png'
