...
DIR_GDAS_HGT250 = DIR_DADOS_BASE / 'GDAS_HGT250'
    ...
    for attempt in range(1, 4):
        try:
            ...
            if tmp.exists():
                tmp.unlink()
            if attempt == 3:
                raise RuntimeError(f'Falha ao baixar GDAS HGT250 após 3 tentativas: {target.name}') from exc
...
def _open_gdas_grb2(path: Path) -> xr.Dataset:
    ...
    # Renomear variável gh → hgt (padrão do projeto)
    if 'gh' in ds.data_vars:
        ds = ds.rename({'gh': 'hgt'})

    # Remover dimensão de nível (único nível: 250mb)
    ...
    ds['hgt'].attrs['long_name'] = 'geopotential height at 250 hPa'

    return ds[['hgt']]


def download_gdas_hgt250_for_date(
    target_date: date,
    hours: Sequence[int] = DEFAULT_SYNOPTIC_HOURS,
    force_redownload: bool = False,
) -> Path:
    """
    Baixa HGT 250mb do GDAS para um dia via NOMADS Grib Filter e salva como NetCDF diário.

    Retorna path do NetCDF com as horas sinóticas concatenadas.
    """
    date_str = target_date.strftime('%Y%m%d')
    nc_path = DIR_GDAS_HGT250 / f'gdas_hgt250_{date_str}.nc'

    if nc_path.exists() and not force_redownload:
        logger.info('GDAS HGT250 {} já existe, pulando download.', date_str)
        return nc_path

    logger.info('Baixando GDAS HGT250 para {} ({} sinóticas)', date_str, len(hours))
    DIR_GDAS_HGT250.mkdir(parents=True, exist_ok=True)

    datasets = []
    for hour in hours:
        grb_path = DIR_GDAS_HGT250 / f'gdas_hgt250_{date_str}_{hour:02d}z.grb2'
    ...
    logger.info('GDAS HGT250 {} salvo: {}', date_str, nc_path)
    return nc_path


def ensure_gdas_hgt250_for_period(
    start: datetime,
    end: datetime,
    hours: Sequence[int] = DEFAULT_SYNOPTIC_HOURS,
    force_redownload: bool = False,
    ...
    """
    Garante NetCDFs de HGT 250mb do GDAS para o período [start, end].
    ...
    while current <= end_date:
        nc_path = download_gdas_hgt250_for_date(current, hours, force_redownload)
        files.append(nc_path)
        current += timedelta(days=1)

    logger.info(
        'GDAS HGT250: {} arquivos para período {} → {}',
