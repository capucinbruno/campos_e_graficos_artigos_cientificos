# app/src/uteis/downloaders_aifs_uv250.py
# -*- coding: utf-8 -*-
"""
Downloader AIFS-single (IA deterministica do ECMWF) de u/v em 250 hPa.

Espelha o ECMWF HRES u/v 250 (mesmo open data / byte-range), mudando so o `model` para
`aifs-single/0p25`. Um NetCDF por dia valido, variaveis 'u'/'v' (m/s).
"""
...
DIR_AIFS_UV250 = DIR_DADOS_BASE / 'AIFS_UV250'


def _download_day(init: datetime, day: date, steps: List[Tuple[int, datetime]], force: bool) -> Path:
    fname = f'aifs_uv250_{init.strftime("%Y%m%d%H")}_valid{day.strftime("%Y%m%d")}.nc'
    nc_path = DIR_AIFS_UV250 / fname
    if nc_path.exists() and not force:
        logger.info('AIFS u/v 250 valido {} (init {}Z) ja existe — pulando.', day, init.hour)
        return nc_path
    DIR_AIFS_UV250.mkdir(parents=True, exist_ok=True)
    tmp = DIR_AIFS_UV250 / f'aifs_uv250_{init.strftime("%Y%m%d%H")}_{day.strftime("%Y%m%d")}_tmp.grb2'
    parts = []
    for step, vt in steps:
        try:
            ...
            logger.warning('  AIFS u/v 250 step {:03d}h ainda nao publicado (404) — pulando', step)
            continue
        parts.append(ds.load())
    if tmp.exists():
        tmp.unlink()
    if not parts:
        logger.warning('AIFS u/v 250 valido {} sem passos publicados — dia ignorado.', day)
        return None
    ds_day = xr.concat(parts, dim='time', coords='minimal', compat='override').sortby('time')
    if nc_path.exists():
        nc_path.unlink()
    save_netcdf(ds_day, nc_path)
    logger.info('AIFS u/v 250 valido {} salvo: {}', day, nc_path.name)
    return nc_path


def ensure_aifs_uv250_fcst_for_period(
    init: datetime, lead_hours: int,
    hours: Sequence[int] = DEFAULT_SYNOPTIC_HOURS, force_redownload: bool = False,
) -> List[Path]:
    """NetCDFs diarios de u/v 250 hPa (m/s) do AIFS-single para [init, init+lead_hours]."""
    ...
    logger.info('AIFS u/v 250: {} arquivos | init {:%Y-%m-%d %H}Z + {}h', len(files), init, lead_hours)
    return files
