# app/src/uteis/downloaders_ecmwf_hgt700.py
# -*- coding: utf-8 -*-
"""
Downloader ECMWF HRES (previsao) de altura geopotencial em 700 hPa via ECMWF Open Data.

Espelha o downloaders_ecmwf_tmp850, mas extrai `gh` @ 700 hPa. Um NetCDF por dia valido,
...
DIR_ECMWF_HGT700 = DIR_DADOS_BASE / 'ECMWF_HGT700'


def _open_hgt700(
    init: datetime, step: int, tmp_path: Path,
    model=None, stream=None, ftype=None, level: int = 700,
) -> xr.Dataset:
...
def _download_day(init: datetime, day: date, steps: List[Tuple[int, datetime]], force: bool) -> Path:
    fname = f'ecmwf_hgt700_{init.strftime("%Y%m%d%H")}_valid{day.strftime("%Y%m%d")}.nc'
    nc_path = DIR_ECMWF_HGT700 / fname
    if nc_path.exists() and not force:
        logger.info('ECMWF Z700 valido {} (init {}Z) ja existe — pulando.', day, init.hour)
        return nc_path
    DIR_ECMWF_HGT700.mkdir(parents=True, exist_ok=True)
    tmp = DIR_ECMWF_HGT700 / f'ecmwf_z700_{init.strftime("%Y%m%d%H")}_{day.strftime("%Y%m%d")}_tmp.grb2'
    parts = []
    for step, vt in steps:
        try:
            ds = _open_hgt700(init, step, tmp).expand_dims(time=[np.datetime64(vt)])
        except StepNotAvailable:
            logger.warning('  ECMWF Z700 step {:03d}h ainda nao publicado (404) — pulando', step)
            continue
        parts.append(ds.load())
    if tmp.exists():
        tmp.unlink()
    if not parts:
        logger.warning('ECMWF Z700 valido {} sem passos publicados — dia ignorado.', day)
        return None
    ds_day = xr.concat(parts, dim='time', coords='minimal', compat='override').sortby('time')
    if nc_path.exists():
        nc_path.unlink()
    save_netcdf(ds_day, nc_path)
    logger.info('ECMWF Z700 valido {} salvo: {}', day, nc_path.name)
    return nc_path


def ensure_ecmwf_hgt700_fcst_for_period(
    init: datetime, lead_hours: int,
    hours: Sequence[int] = DEFAULT_SYNOPTIC_HOURS, force_redownload: bool = False,
) -> List[Path]:
    """NetCDFs diarios de Z700 (m) do ECMWF HRES para [init, init+lead_hours]."""
    ...
    logger.info('ECMWF Z700: {} arquivos | init {:%Y-%m-%d %H}Z + {}h', len(files), init, lead_hours)
    return files
