...
DIR_GDAS_UV925 = DIR_DADOS_BASE / 'GDAS_UV925'
...
def _download_grb2(params: dict, target: Path, timeout: int = 120) -> None:
    ...
    for attempt in range(1, 4):
        try:
            ...
            if attempt == 3:
                raise RuntimeError(f'Falha ao baixar GDAS UV925 após 3 tentativas: {target.name}') from exc
...
def _open_gdas_grb2(path: Path) -> xr.Dataset:
    """Abre GRIB2 filtrado do GDAS e normaliza u/v 925mb para padrão do projeto."""
    datasets = []
    for comp, short in [('u', 'u'), ('v', 'v')]:
        ...
        # Remover dimensão de nível (único nível: 925mb)
        ...
        ds[comp].attrs['long_name'] = f'{comp}-component of wind at 925 hPa'
...
def download_gdas_uv925_for_date(
    target_date: date,
    hours: Sequence[int] = DEFAULT_SYNOPTIC_HOURS,
    force_redownload: bool = False,
) -> Path:
    """
    Baixa u/v 925mb do GDAS para um dia via NOMADS Grib Filter e salva como NetCDF diário.

    Retorna path do NetCDF com as horas sinóticas concatenadas.
    """
    date_str = target_date.strftime('%Y%m%d')
    nc_path = DIR_GDAS_UV925 / f'gdas_uv925_{date_str}.nc'

    if nc_path.exists() and not force_redownload:
        logger.info('GDAS UV925 {} já existe, pulando download.', date_str)
        return nc_path

    logger.info('Baixando GDAS UV925 para {} ({} sinóticas)', date_str, len(hours))
    DIR_GDAS_UV925.mkdir(parents=True, exist_ok=True)

    datasets = []
    for hour in hours:
        grb_path = DIR_GDAS_UV925 / f'gdas_uv925_{date_str}_{hour:02d}z.grb2'
        ...
        if not grb_path.exists() or force_redownload:
            ...
            try:
                ...
                logger.warning('  GDAS UV925 {} {}Z ainda nao publicado (404) — pulando.', date_str, f'{hour:02d}')
                continue
    ...
    if not datasets:  # nenhum ciclo do dia disponivel ainda -> sinaliza p/ o chamador pular o dia
        raise StepNotAvailable(f'gdas_uv925_{date_str}')
    ...
    logger.info('GDAS UV925 {} salvo: {}', date_str, nc_path)
    return nc_path


def ensure_gdas_uv925_for_period(
    start: datetime,
    end: datetime,
    hours: Sequence[int] = DEFAULT_SYNOPTIC_HOURS,
    force_redownload: bool = False,
    ...
    """
    Garante NetCDFs de u/v 925mb do GDAS para o período [start, end].
    ...
    while current <= end_date:
        try:
            nc_path = download_gdas_uv925_for_date(current, hours, force_redownload)
            files.append(nc_path)
        except StepNotAvailable:  # dia (ex.: hoje) ainda sem ciclos publicados -> segue sem ele
            logger.warning('GDAS UV925 {} sem ciclos publicados — dia ignorado.', current)
        current += timedelta(days=1)

    logger.info(
        'GDAS UV925: {} arquivos para período {} → {}',
