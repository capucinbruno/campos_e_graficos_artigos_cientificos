# app/src/uteis/downloaders_gfs_uv850.py
# -*- coding: utf-8 -*-
"""
Downloader GFS (previsao) de u/v em 850 hPa via NOMADS Grib Filter.

Um NetCDF por dia valido, variaveis 'u'/'v' (m/s). Reusa _download_grb2/_steps_for_day e o
parser u/v (`_open_gfs_grb2`, agnostico de nivel) do downloader 200. Espelha o uv250 (nivel 850).
"""
...
DIR_GFS_UV850 = DIR_DADOS_BASE / 'GFS_UV850'
...
def _download_day(init: datetime, day: date, steps: List[Tuple[int, datetime]], force: bool) -> Path:
    fname = f'gfs_uv850_{init.strftime("%Y%m%d%H")}_valid{day.strftime("%Y%m%d")}.nc'
    nc_path = DIR_GFS_UV850 / fname
    if nc_path.exists() and not force:
        logger.info('GFS u/v 850 valido {} (init {}Z) ja existe — pulando.', day, init.hour)
        return nc_path
    DIR_GFS_UV850.mkdir(parents=True, exist_ok=True)
    parts = []
    for fhr, vt in steps:
        grb = DIR_GFS_UV850 / f'gfs_uv850_{init.strftime("%Y%m%d%H")}_f{fhr:03d}.grb2'
        if not grb.exists() or force:
            try:
                _download_grb2(_build_params(init, fhr), grb)
            except StepNotAvailable:
                logger.warning('  GFS u/v 850 f{:03d} ainda nao publicado (404) — pulando passo', fhr)
                continue
        ds = _open_gfs_grb2(grb)[['u', 'v']].expand_dims(time=[np.datetime64(vt)])
        parts.append(ds.load())
    if not parts:
        logger.warning('GFS u/v 850 valido {} sem passos publicados — dia ignorado.', day)
        return None
    ...
    for fhr, _ in steps:
        ...
        if grb.exists():
            grb.unlink()
    logger.info('GFS u/v 850 valido {} salvo: {}', day, nc_path.name)
    return nc_path


def ensure_gfs_uv850_fcst_for_period(
    init: datetime, lead_hours: int,
    hours: Sequence[int] = DEFAULT_SYNOPTIC_HOURS, force_redownload: bool = False,
) -> List[Path]:
    """NetCDFs diarios de u/v 850 hPa (m/s) do GFS para a janela [init, init+lead_hours]."""
    ...
    logger.info('GFS u/v 850: {} arquivos | init {:%Y-%m-%d %H}Z + {}h', len(files), init, lead_hours)
    return files
