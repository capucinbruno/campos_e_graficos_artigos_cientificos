# app/src/uteis/downloaders_era5_altura_geopotencial_700_global_grib.py
# -*- coding: utf-8 -*-
"""
Downloader para ERA5 pressure-levels (GRIB) com:
- geopotential (z) em 700 hPa, domínio global
...
# Logger
# -----------------------------------------------------------------------------
LOGGER = logging.getLogger('ERA5_ALTURA_GEOPOTENCIAL_700_GLOBAL_GRIB')
...
# Diretórios
# -----------------------------------------------------------------------------
DIR_ERA5_BASE = DIR_DADOS_BASE / 'ERA5_ALTURA_GEOPOTENCIAL_700_GLOBAL'
DIR_ERA5_Z700 = DIR_ERA5_BASE / 'z700_hourly_global'
...
VARIABLES_Z700 = ['geopotential']
PRESSURE_LEVEL_Z700 = '700'
...
def _format_incomplete_period_message(
    ...
    if requested_end_day is None:
        return (
            f'[ERA5_ALTURA_GEOPOTENCIAL_700_GLOBAL_GRIB] O período solicitado ainda não está completo no CDS. '
            f'Para {month:02d}/{year:04d}, há dado completo disponível apenas até '
            f'{last_complete_day:02d}/{month:02d}/{year:04d}.'
        )

    return (
        f'[ERA5_ALTURA_GEOPOTENCIAL_700_GLOBAL_GRIB] Você solicitou até {requested_end_day:02d}/{month:02d}/{year:04d}, '
...
def _safe_download_with_cds_interpretation(
    ...
    *,
    ...
    if not days_requested:
        raise RuntimeError("[ERA5_ALTURA_GEOPOTENCIAL_700_GLOBAL_GRIB] Request sem campo 'day'.")
    ...
    try:
        ...
        if _is_cds_no_data_error(msg):
            ...
            if last_complete is None or last_complete <= 0:
                raise RuntimeError(
                    f'[ERA5_ALTURA_GEOPOTENCIAL_700_GLOBAL_GRIB] O CDS indicou disponibilidade parcial para {month:02d}/{year:04d}, '
    ...
    if not _file_ok(tmp_path, MIN_BYTES_GRIB):
        _safe_unlink(tmp_path)
        raise RuntimeError(
            f'[ERA5_ALTURA_GEOPOTENCIAL_700_GLOBAL_GRIB] Arquivo temporário inválido após download: {tmp_path}'
        )
...
# -----------------------------------------------------------------------------
# Downloader principal: geopotential 700 hPa global em GRIB
# -----------------------------------------------------------------------------
def download_era5_altura_geopotencial_700_global_hourly_grib(
    year: int,
    month: int,
    ...
    """
    Baixa ERA5 pressure-levels global em GRIB com geopotential (z) em 700 hPa.
    ...
    _ensure_dir(DIR_ERA5_Z700)

    hours_tag = ''.join(f'{h:02d}' for h in norm_hours)
    fname_nc = f'era5_z700_global_hourly_{year:04d}{month:02d}_h{hours_tag}.nc'
    target_grib = DIR_ERA5_Z700 / fname_nc
    ...
    if end_day is not None and (end_day < 1 or end_day > month_last):
        raise ValueError(
            f'[ERA5_ALTURA_GEOPOTENCIAL_700_GLOBAL_GRIB] end_day inválido ({end_day}) para {month:02d}/{year:04d}.'
        )
    ...
    request = {
        ...
        'variable': VARIABLES_Z700,
        'pressure_level': [PRESSURE_LEVEL_Z700],
        'year': [f'{year:04d}'],
        'month': [f'{month:02d}'],
        'day': days,
        'time': time_list,
    ...
    LOGGER.info(
        'Baixando %s -> %s (z700 global %04d-%02d, dias 01..%s, horas=%s, formato=NetCDF)',
    ...
    LOGGER.info(
        '[OK] z700 global %04d-%02d salvo (%s). Dias efetivos requisitados: 01..%s | Horas=%s | Formato=NetCDF',
        year,
        month,
        final_path,
        used_days[-1],
...
def converter_grib_z_para_altura_geopotencial_netcdf(
    ...
    if arquivo_saida_nc is None:
        stem = arquivo_entrada_grib.stem.replace('_z700_', '_hgt700_')
        if stem == arquivo_entrada_grib.stem:
            stem = arquivo_entrada_grib.stem + '_hgt700'
...
def ensure_era5_altura_geopotencial_700_global_for_period_grib(
    start: datetime,
    end: datetime,
    hours_utc: Sequence[int] | None = None,
    force_redownload: bool = False,
    ...
    for y, m in _iter_year_month_range(start, end):
        ...
        arq = download_era5_altura_geopotencial_700_global_hourly_grib(
            y,
            m,
            end_day=end_day,
            hours_utc=hours_utc,
...
if __name__ == '__main__':
    arquivo_nc = download_era5_altura_geopotencial_700_global_hourly_grib(
        year=2026,
        month=2,
        end_day=20,
        hours_utc=[0, 6, 12, 18],
