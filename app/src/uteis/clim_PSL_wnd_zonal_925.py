...
def get_clim_wnd_zonal_925_path(data_inicial, data_final) -> Path:
    """Garante e retorna path da climatologia PSL de vento zonal (u) em 925mb (com cache local)."""
    data_inicial = str(data_inicial)
    data_final = str(data_final)
    with sync_playwright() as playwright:
        return _run_wnd_zonal_925(playwright, data_inicial, data_final)


def _run_wnd_zonal_925(playwright: Playwright, data_inicial: str, data_final: str) -> Path:
    ...
    file_u = output_dir / f'clim_uzonal925_{mes_ini}{dia_ini}_{mes_fim}{dia_fim}.nc'

    if file_u.exists():
        logger.info('Climatologia PSL u-zonal 925mb já existe, pulando download: {}', file_u)
        return file_u

    logger.info('Baixando climatologia PSL u-zonal 925mb ({}/{} → {}/{})', dia_ini, mes_ini, dia_fim, mes_fim)
    ...
    page.locator('select[name="var"]').select_option('Zonal Wind')
    ...
    url_u = dl_info.value.url

    context.close()
    browser.close()

    resp = httpx.get(url_u)
    if resp.status_code != 200:
        raise RuntimeError(f'Falha no download da climatologia PSL u-zonal 925mb: HTTP {resp.status_code}')
    file_u.write_bytes(resp.content)
    logger.info('Climatologia u-zonal 925mb salva: {}', file_u)
