...
DIR_GDAS_HGT850 = DIR_DADOS_BASE / 'GDAS_HGT850'
    ...
    for attempt in range(1, 4):
        try:
            ...
            if tmp.exists():
                tmp.unlink()
            if attempt == 3:
                raise RuntimeError(f'Falha ao baixar GDAS HGT850 após 3 tentativas: {target.name}') from exc
...
def _open_gdas_grb2(path: Path) -> xr.Dataset:
    ...
    ds['hgt'].attrs['long_name'] = 'geopotential height at 850 hPa'

    return ds[['hgt']]


def download_gdas_hgt850_for_date(
    target_date: date,
    hours: Sequence[int] = DEFAULT_SYNOPTIC_HOURS,
    force_redownload: bool = False,
) -> Path:
    """
    Baixa HGT 850mb do GDAS para um dia via NOMADS Grib Filter e salva como NetCDF diário.

    Retorna path do NetCDF com as horas sinóticas concatenadas.
    """
    date_str = target_date.strftime('%Y%m%d')
    nc_path = DIR_GDAS_HGT850 / f'gdas_hgt850_{date_str}.nc'

    if nc_path.exists() and not force_redownload:
        logger.info('GDAS HGT850 {} já existe, pulando download.', date_str)
        return nc_path

    logger.info('Baixando GDAS HGT850 para {} ({} sinóticas)', date_str, len(hours))
    DIR_GDAS_HGT850.mkdir(parents=True, exist_ok=True)

    datasets = []
    for hour in hours:
        grb_path = DIR_GDAS_HGT850 / f'gdas_hgt850_{date_str}_{hour:02d}z.grb2'
    ...
    logger.info('GDAS HGT850 {} salvo: {}', date_str, nc_path)
    return nc_path


def ensure_gdas_hgt850_for_period(
    start: datetime,
    end: datetime,
    hours: Sequence[int] = DEFAULT_SYNOPTIC_HOURS,
    force_redownload: bool = False,
    ...
    """
    Garante NetCDFs de HGT 850mb do GDAS para o período [start, end].
    ...
    while current <= end_date:
        nc_path = download_gdas_hgt850_for_date(current, hours, force_redownload)
        files.append(nc_path)
        current += timedelta(days=1)

    logger.info(
        'GDAS HGT850: {} arquivos para período {} → {}',
