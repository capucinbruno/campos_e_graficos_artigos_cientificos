...
DIR_GDAS_UV1000 = DIR_DADOS_BASE / 'GDAS_UV1000'
    ...
    for attempt in range(1, 4):
        try:
            ...
            if tmp.exists():
                tmp.unlink()
            if attempt == 3:
                raise RuntimeError(f'Falha ao baixar GDAS UV1000 após 3 tentativas: {target.name}') from exc


def _open_gdas_grb2(path: Path) -> xr.Dataset:
    """Abre GRIB2 filtrado do GDAS e normaliza u/v 1000mb para padrão do projeto."""
    datasets = []
    for comp, short in [('u', 'u'), ('v', 'v')]:
        ...
        ds[comp].attrs['long_name'] = f'{comp}-component of wind at 1000 hPa'
...
def download_gdas_uv1000_for_date(
    target_date: date,
    hours: Sequence[int] = DEFAULT_SYNOPTIC_HOURS,
    force_redownload: bool = False,
) -> Path:
    """Baixa u/v 1000mb do GDAS para um dia via NOMADS Grib Filter e salva como NetCDF diário."""
    date_str = target_date.strftime('%Y%m%d')
    nc_path = DIR_GDAS_UV1000 / f'gdas_uv1000_{date_str}.nc'

    if nc_path.exists() and not force_redownload:
        logger.info('GDAS UV1000 {} já existe, pulando download.', date_str)
        return nc_path

    logger.info('Baixando GDAS UV1000 para {} ({} sinóticas)', date_str, len(hours))
    DIR_GDAS_UV1000.mkdir(parents=True, exist_ok=True)

    datasets = []
    for hour in hours:
        grb_path = DIR_GDAS_UV1000 / f'gdas_uv1000_{date_str}_{hour:02d}z.grb2'
    ...
    logger.info('GDAS UV1000 {} salvo: {}', date_str, nc_path)
    return nc_path


def ensure_gdas_uv1000_for_period(
    start: datetime,
    end: datetime,
    hours: Sequence[int] = DEFAULT_SYNOPTIC_HOURS,
    force_redownload: bool = False,
) -> List[Path]:
    """Garante NetCDFs de u/v 1000mb do GDAS para o período [start, end]."""
    files: List[Path] = []
    current = start.date()
    end_date = end.date()

    while current <= end_date:
        nc_path = download_gdas_uv1000_for_date(current, hours, force_redownload)
        files.append(nc_path)
        current += timedelta(days=1)

    logger.info(
        'GDAS UV1000: {} arquivos para período {} → {}',
