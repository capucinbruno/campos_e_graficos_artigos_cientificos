...
def get_clim_tmptmp2m_path(data_inicial, data_final) -> Path:
    """Garante e retorna path da climatologia PSL de temperatura tmp2m hPa (com cache local)."""
...
def run_temp(playwright: Playwright, data_inicial: str, data_final: str) -> Path:
    ...
    file_name = f'clim_tmptmp2m_{mes_ini}{dia_ini}_{mes_fim}{dia_fim}.nc'
    file_path = output_dir / file_name

    if file_path.exists():
        logger.info('Climatologia PSL tmptmp2m já existe, pulando download: {}', file_path)
        return file_path

    logger.info('Baixando climatologia PSL tmptmp2m ({}/{} → {}/{})', dia_ini, mes_ini, dia_fim, mes_fim)
    ...
    page.locator('select[name="var"]').select_option('2m Air Temperature')
    page.locator('select[name="level"]').select_option('1000mb')
    ...
    if response.status_code != 200:
        raise RuntimeError(f'Falha no download da climatologia PSL tmptmp2m: HTTP {response.status_code}')
