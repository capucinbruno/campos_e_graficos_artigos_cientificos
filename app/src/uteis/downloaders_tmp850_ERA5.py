# app/src/uteis/downloaders_tmp850_ERA5.py
# -*- coding: utf-8 -*-
"""
Downloader para ERA5 pressure-levels (NetCDF) com:
- temperature (t) em 850 hPa, domínio global

Objetivo:
- Baixar dados do CDS com robustez (arquivo .part, validação de tamanho)
...
# Logger
# -----------------------------------------------------------------------------
LOGGER = logging.getLogger('ERA5_TEMPERATURA_850_GLOBAL')
...
# Diretórios
# -----------------------------------------------------------------------------
DIR_ERA5_T850 = DIR_DADOS_BASE / 'ERA5_TEMPERATURA_850_GLOBAL' / 't850_hourly_global'
...
VARIABLES_T850 = ['temperature']
PRESSURE_LEVEL_T850 = '850'
...
def _choose_main_var(ds: xr.Dataset) -> xr.DataArray:
    for vn in ('t', 'temperature', 'tmp'):
        if vn in ds.data_vars:
            return ds[vn]
    for vn in ds.data_vars:
        if np.issubdtype(ds[vn].dtype, np.number):
            return ds[vn]
    raise KeyError(f'Nenhuma variável de temperatura encontrada. Disponíveis: {list(ds.data_vars)}')
...
def download_era5_t850_hourly(
    ...
    """Baixa ERA5 temperatura 850 hPa global como NetCDF mensal."""
    norm_hours = _normalize_hours(hours_utc)
    time_list = _time_list_from_hours(norm_hours)

    _ensure_dir(DIR_ERA5_T850)

    hours_tag = ''.join(f'{h:02d}' for h in norm_hours)
    fname = f'era5_t850_global_hourly_{year:04d}{month:02d}_h{hours_tag}.nc'
    target = DIR_ERA5_T850 / fname
    ...
    request = {
        ...
        'variable': VARIABLES_T850,
        'pressure_level': [PRESSURE_LEVEL_T850],
        'year': [f'{year:04d}'],
        'month': [f'{month:02d}'],
        'day': days,
        'time': time_list,
    ...
    LOGGER.info(
        'Baixando ERA5 t850 global %04d-%02d, dias 01..%s, horas=%s → %s',
    ...
    try:
        client.retrieve(DATASET_ERA5_PRESSURE_LEVELS, request, str(tmp_path))
    except Exception as e:
        msg = str(e)
        if _is_cds_no_data_error(msg):
            ...
            if last_day is None or last_day <= 0:
                raise RuntimeError(
                    f'ERA5 t850: sem dia completo disponível para {month:02d}/{year:04d}.'
                ) from e
            if end_day is not None:
                raise RuntimeError(
                    f'ERA5 t850: solicitado até {end_day:02d}/{month:02d}/{year:04d}, '
                    f'disponível até {last_day:02d}/{month:02d}/{year:04d}.'
                ) from e
            trimmed = [d for d in days if int(d) <= last_day]
            if not trimmed:
                raise RuntimeError(
                    f'ERA5 t850: nenhum dia disponível para {month:02d}/{year:04d}.'
                ) from e
    ...
    if not _file_ok(tmp_path):
        _safe_unlink(tmp_path)
        raise RuntimeError(f'ERA5 t850: arquivo temporário inválido após download: {tmp_path}')

    _safe_unlink(target)
    tmp_path.rename(target)

    LOGGER.info('[OK] ERA5 t850 %04d-%02d salvo: %s', year, month, target)
    return target
...
def ensure_era5_t850_for_period(
    start: datetime,
    end: datetime,
    hours_utc: Sequence[int] | None = None,
    force_redownload: bool = False,
) -> List[Path]:
    """Garante arquivos mensais de temperatura 850 hPa ERA5 para o período [start, end]."""
    files: List[Path] = []
    for y, m in _iter_year_month_range(start, end):
        end_day = end.day if (y == end.year and m == end.month) else None
        arq = download_era5_t850_hourly(y, m, end_day=end_day, hours_utc=hours_utc, force_redownload=force_redownload)
        files.append(arq)
    return files


if __name__ == '__main__':
    path = download_era5_t850_hourly(year=2026, month=2, end_day=20)
