# app/src/uteis/downloaders_gfs_hgt500.py
# -*- coding: utf-8 -*-
"""
Downloader GFS (previsao) de altura geopotencial em 500 hPa via NOMADS Grib Filter.
...
downloaders_gfs_tmp850, mudando so a variavel/nivel (HGT @ 500 hPa).
"""
...
DIR_GFS_HGT500 = DIR_DADOS_BASE / 'GFS_HGT500'
...
def _open_gfs_hgt500(path: Path) -> xr.Dataset:
    """Abre o GRIB2 do HGT@500 e normaliza para 'hgt' (m) em (lat, lon)."""
...
def _download_day(init: datetime, day: date, steps: List[Tuple[int, datetime]], force: bool) -> Path:
    fname = f'gfs_hgt500_{init.strftime("%Y%m%d%H")}_valid{day.strftime("%Y%m%d")}.nc'
    nc_path = DIR_GFS_HGT500 / fname
    if nc_path.exists() and not force:
        logger.info('GFS Z500 valido {} (init {}Z) ja existe — pulando.', day, init.hour)
        return nc_path
    DIR_GFS_HGT500.mkdir(parents=True, exist_ok=True)
    parts = []
    for fhr, vt in steps:
        grb = DIR_GFS_HGT500 / f'gfs_hgt500_{init.strftime("%Y%m%d%H")}_f{fhr:03d}.grb2'
        if not grb.exists() or force:
            try:
                _download_grb2(_build_params(init, fhr), grb)
            except StepNotAvailable:
                logger.warning('  GFS Z500 f{:03d} ainda nao publicado (404) — pulando passo', fhr)
                continue
        ds = _open_gfs_hgt500(grb).expand_dims(time=[np.datetime64(vt)])
        parts.append(ds.load())
    if not parts:
        logger.warning('GFS Z500 valido {} sem passos publicados — dia ignorado.', day)
        return None
    ...
    for fhr, _ in steps:
        ...
        if grb.exists():
            grb.unlink()
    logger.info('GFS Z500 valido {} salvo: {}', day, nc_path.name)
    return nc_path


def ensure_gfs_hgt500_fcst_for_period(
    init: datetime, lead_hours: int,
    hours: Sequence[int] = DEFAULT_SYNOPTIC_HOURS, force_redownload: bool = False,
) -> List[Path]:
    """NetCDFs diarios de Z500 (m) do GFS para a janela [init, init+lead_hours]."""
    ...
    logger.info('GFS Z500: {} arquivos | init {:%Y-%m-%d %H}Z + {}h', len(files), init, lead_hours)
    return files
