# app/src/uteis/downloaders_era5_vento_pressao.py
# -*- coding: utf-8 -*-
"""
Downloaders para ERA5 single-levels (NetCDF) com:
...
Padrão inspirado em downloaders_smas_era5.py
"""
...
# Isso resolve casos em que a UI ainda mostra o último dia desabilitado.
...
def _apply_cds_ui_conservative_cutoff_if_needed(
    *,
    ...
    Isso evita reportar "18" quando a UI ainda mostra "17" como último dia liberado.
    """
    ...
    # rebaixa 1 dia, respeitando limite mínimo
...
def _summarize_synoptic_coverage_in_file(
    ...
    Retorna dict com:
      ...
      - n_complete_days
      - last_any_date
      - last_complete_date_raw  (sem regra conservadora da UI)
      - hours_count_by_day
...
def _last_complete_synoptic_date_in_file(
    ...
    """
    Retorna a última data com todas as horas requeridas no arquivo (sem regra conservadora UI).
    """
    try:
...
def _existing_file_satisfies_period(
    ...
    *,
    ...
    - precisa ter último dia completo >= dia alvo (sem regra UI conservadora)
      (a regra UI conservadora é aplicada nos casos de erro/indisponibilidade)
    """
...
def _last_complete_day_from_latest_dt(
    ...
    """
    Converte "latest available datetime" em "último dia completo" para as horas pedidas.

    Exemplo:
    - Se pede 00/06/12/18, o dia só é completo se latest_dt.hour >= 18
    - Se pede só 00, o dia é completo se latest_dt.hour >= 0
    """
...
def _validate_downloaded_file_coverage(
    ...
    *,
    ...
    """
    Valida o conteúdo REAL do arquivo baixado.

    Se requested_end_day foi informado e o arquivo não cobre até lá com todas as horas
    requeridas, levanta erro informando o último dia completo disponível.

    IMPORTANTÍSSIMO:
    - Usa a cobertura real do arquivo.
    - Depois aplica regra conservadora alinhada à UI do CDS (mês corrente), rebaixando 1 dia
      quando necessário.
    """
    summary = _summarize_synoptic_coverage_in_file(path_nc, required_hours_utc)
...
def _safe_download_with_cds_interpretation(
    ...
    *,
    ...
    # Validação pós-download (conteúdo real + regra UI conservadora)
...
# -----------------------------------------------------------------------------
# Downloader principal: MSLP + U100 + V100 (NetCDF)
# -----------------------------------------------------------------------------
def download_era5_mslp_u100_v100_hourly(
    year: int,
    month: int,
    ...
    """
    Baixa ERA5 single-levels em NetCDF com mslp/u100/v100.
    ...
    end_day : int | None
        ...
        Se None, tenta o mês todo (com auto-trim se o CDS ainda não tiver o mês completo).
    area : [N, W, S, E]
        Bounding box. Padrão = [10, -80, -40, 15]
...
def ensure_era5_mslp_u100_v100_for_period(
    start: datetime,
    end: datetime,
    area: Sequence[float] | None = None,
    hours_utc: Sequence[int] | None = None,
    ...
    for y, m in _iter_year_month_range(start, end):
        ...
        files.append(
            download_era5_mslp_u100_v100_hourly(
                y,
                m,
                end_day=end_day,
                area=area,
...
if __name__ == '__main__':
    ...
    arquivo = download_era5_mslp_u100_v100_hourly(
        year=2026,
        month=2,
        end_day=20,
        area=[10, -80, -40, 15],
