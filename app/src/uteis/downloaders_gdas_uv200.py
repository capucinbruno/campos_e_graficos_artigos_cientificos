...
def download_gdas_uv200_for_date(
    ...
    for hour in hours:
        ...
        if not grb_path.exists() or force_redownload:
            ...
            try:
                ...
                logger.warning('  GDAS UV200 {} {}Z ainda nao publicado (404) — pulando.', date_str, f'{hour:02d}')
                continue
    ...
    if not datasets:  # nenhum ciclo do dia disponivel ainda -> sinaliza p/ o chamador pular o dia
        raise StepNotAvailable(f'gdas_uv200_{date_str}')
...
def ensure_gdas_uv200_for_period(
    ...
    while current <= end_date:
        try:
            ...
            logger.warning('GDAS UV200 {} sem ciclos publicados — dia ignorado.', current)
