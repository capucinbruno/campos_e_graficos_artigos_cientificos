...
def get_clim_geop850_path(data_inicial, data_final) -> Path:
...
def run_geop(playwright: Playwright, data_inicial: str, data_final: str) -> Path:
    ...
    file_name = f'clim_geop850_{mes_ini}{dia_ini}_{mes_fim}{dia_fim}.nc'
    file_path = output_dir / file_name

    if file_path.exists():
        logger.info('Climatologia PSL geop850 já existe, pulando download: {}', file_path)
        return file_path

    logger.info('Baixando climatologia PSL geop850 ({}/{} → {}/{})', dia_ini, mes_ini, dia_fim, mes_fim)
