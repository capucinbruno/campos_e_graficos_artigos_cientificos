...
def get_clim_geop200_path(data_inicial, data_final) -> Path:
    """Garante e retorna path da climatologia PSL para o período (com cache local)."""
    data_inicial = str(data_inicial)
    data_final = str(data_final)
    with sync_playwright() as playwright:
        return run_geop(playwright, data_inicial, data_final)
...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        run_geop(playwright, data_inicial, data_final)


def run_geop(playwright: Playwright, data_inicial: str, data_final: str) -> Path:
    ...
    file_name = f'clim_geop200_{mes_ini}{dia_ini}_{mes_fim}{dia_fim}.nc'
    file_path = output_dir / file_name

    if file_path.exists():
        logger.info('Climatologia PSL geop200 já existe, pulando download: {}', file_path)
        return file_path

    logger.info('Baixando climatologia PSL geop200 ({}/{} → {}/{})', dia_ini, mes_ini, dia_fim, mes_fim)

    # PSL espera inteiros sem zero à esquerda nos selects de mês/dia
    ...
    page.locator('select[name="var"]').select_option('Geopotential Height')
    page.locator('select[name="level"]').select_option('200mb')
    ...
    if response.status_code != 200:
        raise RuntimeError(f'Falha no download da climatologia PSL: HTTP {response.status_code}')
