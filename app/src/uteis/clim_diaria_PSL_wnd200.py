...
def get_clim_wnd200_paths(data_inicial, data_final) -> tuple[Path, Path]:
    """Garante e retorna (path_u, path_v) da climatologia PSL vento 200mb (com cache local)."""
    data_inicial = str(data_inicial)
    data_final = str(data_final)
    with sync_playwright() as playwright:
        return _run_wnd200(playwright, data_inicial, data_final)


def _run_wnd200(playwright: Playwright, data_inicial: str, data_final: str) -> tuple[Path, Path]:
    ...
    file_u = output_dir / f'clim_u200_{mes_ini}{dia_ini}_{mes_fim}{dia_fim}.nc'
    file_v = output_dir / f'clim_v200_{mes_ini}{dia_ini}_{mes_fim}{dia_fim}.nc'

    if file_u.exists() and file_v.exists():
        logger.info('Climatologia PSL u/v 200mb já existe, pulando download: {}', output_dir)
        return file_u, file_v

    logger.info('Baixando climatologia PSL u/v 200mb ({}/{} → {}/{})', dia_ini, mes_ini, dia_fim, mes_fim)
    ...
    for url, path, comp in [(url_u, file_u, 'u'), (url_v, file_v, 'v')]:
        ...
        logger.info('Climatologia {} 200mb salva: {}', comp, path)
