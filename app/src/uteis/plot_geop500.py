# app/src/uteis/plot_geop500.py
# -*- coding: utf-8 -*-
"""
Download e processamento de altura geopotencial 500 hPa (ERA5/GDAS) para anomalia.

Pipeline:
1. Baixa dados ERA5 de geopotencial (z) em 500 hPa via CDS (GRIB)
2. Converte geopotencial (m2/s2) para altura geopotencial (m)
3. Concatena arquivos mensais e calcula media diaria
4. Carrega climatologia 1991-2020 de geopotencial 500 hPa
5. Calcula anomalia = media_periodo - media_climatologica
6. Salva resultado em dados/geop500.nc (variavel 'hgt', 1 timestep)

Chamado por: scripts/s01_geop500_anom.py via `from app.src.uteis.plot_geop500 import main`
"""
...
# Módulos locais
from app.src.uteis.clim_PSL_geop500 import get_clim_geop500_path
...
# Logger
# -----------------------------------------------------------------------------
LOGGER = logging.getLogger('PLOT_GEOP500')
...
def main() -> None:
    """
    Download, processamento e calculo de anomalia de geopotencial 500 hPa.
    ...
    Resultado: dados/geop500.nc com variavel 'hgt' (anomalia em metros, 1 timestep).
    """
    ...
    LOGGER.info('=' * 70)
    LOGGER.info('PLOT_GEOP500: Download e anomalia geopotencial 500 hPa')
    ...
    if era5_period:
        LOGGER.info('Etapa 2a: Download ERA5 geopotencial 500 hPa')
        era5_files = ensure_era5_altura_geopotencial_500_global_for_period_grib(
            start=era5_period[0],
            end=era5_period[1],
    ...
    if gdas_period:
        LOGGER.info('Etapa 2b: Download GDAS HGT 500 hPa (NOMADS)')
        gdas_files = ensure_gdas_hgt500_for_period(
            start=gdas_period[0],
            end=gdas_period[1],
            force_redownload=force,
        )
    ...
    LOGGER.info('Etapa 4: Climatologia PSL geopotencial 500 hPa')
    clim_path = get_clim_geop500_path(settings.DATA_INICIAL, settings.DATA_FINAL)
    ...
    anomaly.name = 'hgt'
    anomaly.attrs['long_name'] = 'geopotential height anomaly at 500 hPa'
    ...
    output_path = DIR_DADOS_BASE / 'geop500.nc'
    ...
    LOGGER.info('=' * 70)
    LOGGER.info('PLOT_GEOP500: Concluido com sucesso')
    LOGGER.info('=' * 70)
