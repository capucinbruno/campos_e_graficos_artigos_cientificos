...
DIR_GDAS_UV850 = DIR_DADOS_BASE / 'GDAS_UV850'
...
def _open_gdas_grb2(path: Path) -> xr.Dataset:
    """Abre GRIB2 filtrado do GDAS e normaliza u/v 850mb para padrão do projeto."""
    datasets = []
    for comp, short in [('u', 'u'), ('v', 'v')]:
        ...
        ds[comp].attrs['long_name'] = f'{comp}-component of wind at 850 hPa'
...
def download_gdas_uv850_for_date(
    target_date: date,
    hours: Sequence[int] = DEFAULT_SYNOPTIC_HOURS,
    force_redownload: bool = False,
) -> Path:
    """
    Baixa u/v 850mb do GDAS para um dia via NOMADS Grib Filter e salva como NetCDF diário.

    Retorna path do NetCDF com as horas sinóticas concatenadas.
    """
    date_str = target_date.strftime('%Y%m%d')
    nc_path = DIR_GDAS_UV850 / f'gdas_uv850_{date_str}.nc'

    if nc_path.exists() and not force_redownload:
        logger.info('GDAS UV850 {} já existe, pulando download.', date_str)
        return nc_path

    logger.info('Baixando GDAS UV850 para {} ({} sinóticas)', date_str, len(hours))
    DIR_GDAS_UV850.mkdir(parents=True, exist_ok=True)
    ...
    logger.info('GDAS UV850 {} salvo: {}', date_str, nc_path)
    return nc_path


def ensure_gdas_uv850_for_period(
    start: datetime,
    end: datetime,
    hours: Sequence[int] = DEFAULT_SYNOPTIC_HOURS,
    force_redownload: bool = False,
    ...
    """
    Garante NetCDFs de u/v 850mb do GDAS para o período [start, end].
    ...
    logger.info(
        'GDAS UV850: {} arquivos para período {} → {}',
