# app/src/uteis/downloaders_wind1000.py
# -*- coding: utf-8 -*-
"""
...
# Logger
# -----------------------------------------------------------------------------
LOGGER = logging.getLogger('ERA5_UV1000')
...
# Diretórios
# -----------------------------------------------------------------------------
DIR_ERA5_BASE = DIR_DADOS_BASE / 'ERA5_UV1000'
DIR_ERA5_UV1000_HOURLY = DIR_ERA5_BASE / 'uv1000_hourly'
...
def _format_incomplete_period_message(
    ...
    if requested_end_day is None:
        return (
            f'[ERA5_UV1000] O período solicitado ainda não está completo no CDS. '
            f'Para {month:02d}/{year:04d}, há dado completo disponível apenas até '
            f'{last_complete_day:02d}/{month:02d}/{year:04d}.'
        )
    return (
        f'[ERA5_UV1000] Você solicitou até {requested_end_day:02d}/{month:02d}/{year:04d}, '
...
def _safe_download_with_cds_interpretation(
    ...
    *,
    ...
    if not days_requested:
        raise RuntimeError("[ERA5_UV1000] Request sem campo 'day'.")
    ...
    try:
        ...
        if _is_cds_no_data_error(msg):
            ...
            if last_complete is None or last_complete <= 0:
                raise RuntimeError(
                    f'[ERA5_UV1000] O CDS indicou disponibilidade parcial para {month:02d}/{year:04d}, '
    ...
    if not _file_ok(tmp_path, MIN_BYTES_FILE):
        _safe_unlink(tmp_path)
        raise RuntimeError(f'[ERA5_UV1000] Arquivo temporário inválido após download: {tmp_path}')
...
# -----------------------------------------------------------------------------
# Downloader principal: U/V em 1000 hPa (NetCDF)
# -----------------------------------------------------------------------------
def _download_single_variable(
    ...
    _ensure_dir(DIR_ERA5_UV1000_HOURLY)

    hours_tag = ''.join(f'{h:02d}' for h in norm_hours)
    grid_tag = grid.replace('/', 'x').replace('.', '')
    var_short = variable.split('_', maxsplit=1)[0]  # 'u' ou 'v'
    fname_nc = f'era5_{var_short}1000_hourly_{year:04d}{month:02d}_h{hours_tag}_g{grid_tag}.nc'
    target_file = DIR_ERA5_UV1000_HOURLY / fname_nc
    ...
    if end_day is not None and (end_day < 1 or end_day > month_last):
        raise ValueError(f'[ERA5_UV1000] end_day inválido ({end_day}) para {month:02d}/{year:04d}.')
    ...
    request = {
        ...
        'day': days,
        'time': time_list,
        'pressure_level': [PRESSURE_LEVEL_1000],
        'data_format': 'netcdf',
    ...
    LOGGER.info(
        'Baixando %s -> %s (%s 1000 hPa %04d-%02d, dias 01..%s, horas=%s)',
    ...
    LOGGER.info(
        '[OK] %s1000 %04d-%02d salvo (%s). Dias: 01..%s | Horas=%s',
        var_short, year, month, final_path, used_days[-1], ','.join(time_list),
    )
    return final_path


def download_era5_uv1000_hourly(
    year: int,
    month: int,
    ...
    """Baixa ERA5 pressure-levels global em NetCDF com u/v em 1000 hPa."""
    norm_hours = _normalize_hours(hours_utc)
    _ensure_dir(DIR_ERA5_UV1000_HOURLY)

    hours_tag = ''.join(f'{h:02d}' for h in norm_hours)
    grid_tag = grid.replace('/', 'x').replace('.', '')
    fname_merged = f'era5_uv1000_hourly_{year:04d}{month:02d}_h{hours_tag}_g{grid_tag}.nc'
    target_merged = DIR_ERA5_UV1000_HOURLY / fname_merged
    ...
    LOGGER.info('Iniciando download de u e v 1000 hPa (%04d-%02d, grid=%s)', year, month, grid)

    var_paths: Dict[str, Path] = {}
    for var in VARIABLES_UV1000:
    ...
    LOGGER.info('[OK] uv1000 %04d-%02d mesclado -> %s', year, month, target_merged.name)
    return target_merged
...
def ensure_era5_uv1000_for_period(
    start: datetime,
    end: datetime,
    hours_utc: Sequence[int] | None = None,
    grid: str = '1.0/1.0',
