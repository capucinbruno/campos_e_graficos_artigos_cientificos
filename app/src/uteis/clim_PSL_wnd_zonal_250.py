...
def get_clim_wnd_zonal_250_path(data_inicial, data_final) -> Path:
    """Garante e retorna path da climatologia PSL de vento zonal (u) em 250mb (com cache local)."""
    data_inicial = str(data_inicial)
    data_final = str(data_final)
    with sync_playwright() as playwright:
        return _run_wnd_zonal_250(playwright, data_inicial, data_final)


def _run_wnd_zonal_250(playwright: Playwright, data_inicial: str, data_final: str) -> Path:
    ...
    file_u = output_dir / f'clim_uzonal250_{mes_ini}{dia_ini}_{mes_fim}{dia_fim}.nc'

    if file_u.exists():
        logger.info('Climatologia PSL u-zonal 250mb já existe, pulando download: {}', file_u)
        return file_u

    logger.info('Baixando climatologia PSL u-zonal 250mb ({}/{} → {}/{})', dia_ini, mes_ini, dia_fim, mes_fim)
    ...
    if resp.status_code != 200:
        raise RuntimeError(f'Falha no download da climatologia PSL u-zonal 250mb: HTTP {resp.status_code}')
    file_u.write_bytes(resp.content)
    logger.info('Climatologia u-zonal 250mb salva: {}', file_u)
