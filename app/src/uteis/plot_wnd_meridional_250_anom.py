# app/src/uteis/plot_wnd_meridional_250_anom.py
# -*- coding: utf-8 -*-
"""
Download e processamento de vento meridional 250 hPa (ERA5/GDAS) para anomalia de V250.
...
4. Calcula anomalia = v_médio_período - climatologia_v_PSL
5. Salva resultado em dados/wnd_meridional_250_anom.nc

Chamado por: scripts/s17_wnd250_meridional.py
"""
...
from app.src.uteis.clim_PSL_wnd_meridional_250 import get_clim_wnd_meridional_250_path
...
LOGGER = logging.getLogger('PLOT_WND_MERIDIONAL_250')
...
ERA5_LATENCY_DAYS = 7
OUTPUT_FILE = DIR_DADOS_BASE / 'wnd_meridional_250_anom.nc'


# -----------------------------------------------------------------------------
# Utilitários (idênticos ao plot_wnd_zonal_250_anom)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Streaming accumulator — apenas v-meridional
# -----------------------------------------------------------------------------
def _compute_period_mean_v_streaming(
    files: Sequence[Path],
    required_hours: Sequence[int] = DEFAULT_SYNOPTIC_HOURS,
    dt_ini: Optional[datetime] = None,
    dt_fim: Optional[datetime] = None,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Calcula média do período para v-meridional processando um arquivo por vez."""
    ...
    for fp in files:
        ...
        try:
            ...
            t = xr.DataArray(da_v['time'].values, dims=['time'])
            ...
            valid = da_v_daily.notnull().any(dim=['lat', 'lon'])
            da_v_daily = da_v_daily.isel(time=valid.values)
            n_days_file = da_v_daily.sizes['time']
            if n_days_file == 0:
                continue
            ...
            if sum_v is None:
                ...
                ref_lat = da_v_daily['lat'].values.copy()
                ref_lon = da_v_daily['lon'].values.copy()
            elif da_v_daily.shape[1:] != sum_v.shape:
    ...
    if sum_v is None or total_days == 0:
    ...
    LOGGER.info(
        'Média do período: %d dias | v=[%.2f, %.2f] m/s',
        total_days, float(np.nanmin(v_mean)), float(np.nanmax(v_mean)),
    )
    return v_mean, ref_lat, ref_lon


# -----------------------------------------------------------------------------
# Climatologia PSL — v-meridional 250mb
# -----------------------------------------------------------------------------
def _load_psl_clim_v(path: Path) -> xr.DataArray:
    if not path.exists():
        raise FileNotFoundError(f'Climatologia PSL v-meridional 250mb não encontrada: {path}')

    ds = xr.open_dataset(path, engine='netcdf4')
    ds = _normalize_latlon_names(ds)

    da = None
    for vname in ('vwnd', 'v', 'v_component_of_wind'):
        if vname in ds.data_vars:
            da = ds[vname]
            break
    if da is None:
        da = next(iter(ds.data_vars.values()))
        LOGGER.warning('Usando variável %s como v-meridional da climatologia PSL.', da.name)
    ...
    ds_tmp = _normalize_lon(da.to_dataset(name='v'))
    da = ds_tmp['v']
    LOGGER.info(
        'Climatologia PSL v-meridional 250mb: shape=%s | lon=[%.1f, %.1f]',
...
def main() -> None:
    """
    Download, processamento e cálculo de anomalia de vento meridional 250 hPa.
    ...
    Resultado: dados/wnd_meridional_250_anom.nc com variável:
    - v_anom_mean: anomalia do vento meridional (m/s)
    """
    ...
    LOGGER.info('=' * 70)
    LOGGER.info('PLOT_WND_MERIDIONAL_250: Anomalia vento meridional 250 hPa')
    ...
    # 2. Download (mesmo arquivo uv250 usado pelo s16 — não baixa de novo se já existir)
    all_files = []
    if era5_period:
    ...
    # 3. Média streaming (v apenas)
    LOGGER.info('Etapa 3: Calculando média do período em streaming')
    v_mean, ref_lat, ref_lon = _compute_period_mean_v_streaming(
        all_files,
    ...
    # 4. Climatologia PSL
    LOGGER.info('Etapa 4: Climatologia PSL v-meridional 250mb')
    clim_v_path = get_clim_wnd_meridional_250_path(settings.DATA_INICIAL, settings.DATA_FINAL)
    clim_v_da = _load_psl_clim_v(clim_v_path)
    clim_v_regrid = _interp_psl_clim(clim_v_da, ref_lat, ref_lon)

    # 5. Anomalia
    LOGGER.info('Etapa 5: Calculando anomalia v-meridional')

    if ref_lat[0] < ref_lat[-1]:
        v_period_da = v_period_da.sortby('lat', ascending=False)
        ref_lat = v_period_da['lat'].values

    v_anom_da = v_period_da - clim_v_regrid
    LOGGER.info(
        'Anomalia v-meridional: min=%.2f, max=%.2f m/s',
        float(v_anom_da.min()), float(v_anom_da.max()),
    )

    # 6. Salvar
    LOGGER.info('Etapa 6: Salvando wnd_meridional_250_anom.nc')
    DIR_DADOS_BASE.mkdir(parents=True, exist_ok=True)

    ds_out = xr.Dataset({
        'v_anom_mean': xr.DataArray(
            v_anom_da.values.astype(np.float32),
            dims=['lat', 'lon'],
            coords={'lat': ref_lat, 'lon': ref_lon},
            attrs={'long_name': 'meridional wind anomaly 250 hPa', 'units': 'm s-1'},
        )
    })

    output_path = DIR_DADOS_BASE / 'wnd_meridional_250_anom.nc'
