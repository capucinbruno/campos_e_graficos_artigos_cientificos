# app/src/uteis/downloaders_aifs_hgt500.py
# -*- coding: utf-8 -*-
"""
Downloader AIFS-single (IA deterministica do ECMWF) de altura geopotencial em 500 hPa.

Espelha o ECMWF HRES Z500 (mesmo open data / byte-range), mudando so o `model` para
...
DIR_AIFS_HGT500 = DIR_DADOS_BASE / 'AIFS_HGT500'


def _download_day(init: datetime, day: date, steps: List[Tuple[int, datetime]], force: bool) -> Path:
    fname = f'aifs_hgt500_{init.strftime("%Y%m%d%H")}_valid{day.strftime("%Y%m%d")}.nc'
    nc_path = DIR_AIFS_HGT500 / fname
    if nc_path.exists() and not force:
        logger.info('AIFS Z500 valido {} (init {}Z) ja existe — pulando.', day, init.hour)
        return nc_path
    DIR_AIFS_HGT500.mkdir(parents=True, exist_ok=True)
    tmp = DIR_AIFS_HGT500 / f'aifs_z500_{init.strftime("%Y%m%d%H")}_{day.strftime("%Y%m%d")}_tmp.grb2'
    parts = []
    for step, vt in steps:
        try:
            ...
            logger.warning('  AIFS Z500 step {:03d}h ainda nao publicado (404) — pulando', step)
            continue
        parts.append(ds.load())
    if tmp.exists():
        tmp.unlink()
    if not parts:
        logger.warning('AIFS Z500 valido {} sem passos publicados — dia ignorado.', day)
        return None
    ds_day = xr.concat(parts, dim='time', coords='minimal', compat='override').sortby('time')
    if nc_path.exists():
        nc_path.unlink()
    save_netcdf(ds_day, nc_path)
    logger.info('AIFS Z500 valido {} salvo: {}', day, nc_path.name)
    return nc_path


def ensure_aifs_hgt500_fcst_for_period(
    init: datetime, lead_hours: int,
    hours: Sequence[int] = DEFAULT_SYNOPTIC_HOURS, force_redownload: bool = False,
) -> List[Path]:
    """NetCDFs diarios de Z500 (m) do AIFS-single para [init, init+lead_hours]."""
    ...
    logger.info('AIFS Z500: {} arquivos | init {:%Y-%m-%d %H}Z + {}h', len(files), init, lead_hours)
    return files
