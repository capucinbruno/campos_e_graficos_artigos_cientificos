# app/src/uteis/downloaders_gefs_hgt500.py
# -*- coding: utf-8 -*-
"""
Downloader GEFS (previsao) de altura geopotencial em 500 hPa via NOMADS Grib Filter.

Espelha o downloaders_gefs_tmp850, mas extrai HGT @ 500 hPa (MEDIA DO ENSEMBLE, membro
`geavg`, no mesmo arquivo pgrb2a 0.5°). Um NetCDF por dia valido, variavel 'hgt' (m).
Reusa os helpers do downloader GEFS 200 e o parser de HGT do GFS Z500.
"""
...
# Parser de GRIB do HGT@500 (model-agnostico) reusado do downloader GFS Z500.
from app.src.uteis.downloaders_gfs_hgt500 import _open_gfs_hgt500 as _open_gefs_hgt500

logger = get_logger(__name__)

DIR_GEFS_HGT500 = DIR_DADOS_BASE / 'GEFS_HGT500'
...
def _download_day(init: datetime, day: date, steps: List[Tuple[int, datetime]], force: bool):
    fname = f'gefs_hgt500_{init.strftime("%Y%m%d%H")}_valid{day.strftime("%Y%m%d")}.nc'
    nc_path = DIR_GEFS_HGT500 / fname
    if nc_path.exists() and not force:
        logger.info('GEFS Z500 valido {} (init {}Z) ja existe — pulando.', day, init.hour)
        return nc_path
    DIR_GEFS_HGT500.mkdir(parents=True, exist_ok=True)
    parts = []
    for fhr, vt in steps:
        grb = DIR_GEFS_HGT500 / f'gefs_hgt500_{init.strftime("%Y%m%d%H")}_f{fhr:03d}.grb2'
        if not grb.exists() or force:
            try:
                _download_grb2(_build_params(init, fhr), grb)
            except StepNotAvailable:
                logger.warning('  GEFS Z500 f{:03d} ainda nao publicado (404) — pulando passo', fhr)
                continue
        ds = _open_gefs_hgt500(grb).expand_dims(time=[np.datetime64(vt)])
        parts.append(ds.load())
    if not parts:
        logger.warning('GEFS Z500 valido {} sem passos publicados — dia ignorado.', day)
        return None
    ...
    for fhr, _ in steps:
        ...
        if grb.exists():
            grb.unlink()
    logger.info('GEFS Z500 valido {} salvo: {}', day, nc_path.name)
    return nc_path


def ensure_gefs_hgt500_fcst_for_period(
    init: datetime, lead_hours: int,
    hours: Sequence[int] = DEFAULT_SYNOPTIC_HOURS, force_redownload: bool = False,
) -> List[Path]:
    """NetCDFs diarios de Z500 (m) do GEFS (media do ensemble) para [init, init+lead_hours]."""
    ...
    logger.info('GEFS Z500: {} arquivos | init {:%Y-%m-%d %H}Z + {}h', len(files), init, lead_hours)
    return files
